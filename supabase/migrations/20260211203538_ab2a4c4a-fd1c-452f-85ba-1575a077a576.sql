...
BEGIN
  ...
  -- Check if a profile exists with the same email (migration in progress)
  IF NEW.email IS NOT NULL AND EXISTS (SELECT 1 FROM public.user_profiles WHERE email = NEW.email) THEN
    -- Update existing profile with auth_id instead of creating a new one
    UPDATE public.user_profiles 
    SET auth_id = NEW.id, auth_migrated = true, updated_at = now()
